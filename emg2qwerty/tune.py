...
def train_once(trial, config: DictConfig):
    ...
    module = instantiate(
        ...
        gru_hidden_size=trial.suggest_int('gru_hidden_size', 16, 256, step=16),
        num_gru_layers=trial.suggest_int('num_gru_layers', 1, 16),
        _recursive_=False,
    )
    ...
    return val_metrics[0]['val/CER']


@hydra.main(version_base=None, config_path="../config", config_name="base")
def main(cfg):
    tuner = optuna.create_study(direction="minimize")
    tuner.optimize(lambda trial: train_once(trial, cfg), n_trials=15)

    print(f'best trial: {tuner.best_trial.value}')
    print(f'best params: {tuner.best_trial.params}')
